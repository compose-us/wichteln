...
describe('validate the array that randomizeParticipants func returned', () => {
	...
	it('does NOT contain falsey values', () => {
		expect(randomizeParticipants(demoParticipants)).toEqual(
			expect.not.arrayContaining([{ firstName: '', lastName: '' }])
		);
		expect(randomizeParticipants(demoParticipants)).toEqual(
			expect.not.arrayContaining([{ firstName: undefined, lastName: undefined }])
		);
	});
});
